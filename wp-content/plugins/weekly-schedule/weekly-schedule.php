	...
	class WS_Admin {
		...
		function stylesheet_config_page() {
			...
			<div class="wrap">
				...
				<a target='wsinstructions' href='https://wordpress.org/extend/plugins/weekly-schedule/installation/'>Installation Instructions</a> |
				<a href='https://wordpress.org/extend/plugins/weekly-schedule/faq/' target='llfaq'>FAQ</a> |
				<a href='https://ylefebvre.github.io/contact-me'>Contact the Author</a><br />
		...
		function config_page() {
			...
			if ( isset( $_POST['submit'] ) ) {
				...
				echo '<div id="message" class="updated fade"><p><strong>Weekly Schedule: Schedule ' . intval( $schedule ) . ' Updated</strong></div>';
			}
			if ( isset( $_POST['submitgen'] ) ) {
				...
				if ( $successfulimport > 0 ) {
					echo '<div id="message" class="updated fade"><p><strong>Successfully imported ' . intval( $successfulimport ) . ' record(s) from ' . intval( $filerow - 1 ) . ' line(s) in import file</strong></div>';
				}
			...
			<a target='wsinstructions' href='https://wordpress.org/extend/plugins/weekly-schedule/installation/'>Installation Instructions</a> |
			<a href='https://wordpress.org/extend/plugins/weekly-schedule/faq/' target='llfaq'>FAQ</a> |
			<a href='https://ylefebvre.github.io/contact-me'>Contact the Author</a><br /><br />
						...
						<tr>
							<td style='padding: 8px; vertical-align: top'>
								...
								<?php } ?>
									...
									<tr>
										<td>Display flat schedule for mobile displays</td>
										<td>
			...
			<div style='padding-top: 15px;clear:both'>
				<fieldset style='border:1px solid #CCC;padding:10px'>
					...
					<FORM name="scheduleselection">
						...
						<SELECT name="schedulelist" style='width: 300px'>
							...
							for ( $counter = 1; $counter <= $numberofschedules; $counter ++ ): ?>
								...
								<option value="<?php echo intval( $counter ) ?>" <?php if ( $schedule == $counter ) {
									echo 'SELECTED';
								} ?>>Schedule <?php echo intval( $counter ) ?><?php if ( !empty( $tempoptions ) ) {
										echo " (" . esc_html( $tempoptions['schedulename'] ) . ")";
									} ?></option>
							<?php endfor; ?>
						</SELECT>
						<INPUT type="button" name="go" value="Go!" onClick="window.location= '?page=weekly-schedule&amp;settings=<?php echo esc_html( $adminpage ); ?>&amp;schedule=' + document.scheduleselection.schedulelist.options[document.scheduleselection.schedulelist.selectedIndex].value">
						Copy from:
						...
						<INPUT type="button" name="copy" value="Copy!" onClick="window.location= '?page=weekly-schedule&amp;copy=<?php echo intval( $schedule ); ?>&source=' + document.scheduleselection.copysource.options[document.scheduleselection.copysource.selectedIndex].value">
						<br />
						<br />
			...
			<legend style='padding: 0 5px 0 5px;'>
				<strong>Settings for Schedule <?php echo intval( $schedule ); ?> - <?php echo esc_html( $options['schedulename'] ); ?></strong>
			</legend>
			<?php if (( empty( $adminpage ) ) || ( $adminpage == "general" )): ?>
			<a href="?page=weekly-schedule&amp;settings=general&amp;schedule=<?php echo intval( $schedule ); ?>"><strong>General Settings</strong></a> |
			<a href="?page=weekly-schedule&amp;settings=categories&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Schedule Categories</a> |
			<a href="?page=weekly-schedule&amp;settings=items&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Schedule Items</a> |
			<a href="?page=weekly-schedule&amp;settings=days&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Days Labels</a><br /><br />
			...
			<table>
				<tr>
					...
					<td><select style="width: 200px" name='timeformat'>
							<?php $descriptions = array( "24hours" => "24 Hours (e.g., 6h00, 17h30)", "24hourslz" => "24 Hours with leading zero (e.g., 06h00, 17h30)",
								"24hourscolon" => "24 Hours with colon (e.g., 6:00, 17:30)", "24hourslzcolon" => "24 Hours with leading zero and colon (e.g., 06:00, 17:30)",
								"12hours" => "12 Hours (e.g., 6:00am, 5:30pm)", "12hoursspace" => "12 Hours with space (e.g., 6:00 am, 5:30 pm)",
								"12hoursno0" => "12 Hours omitting 0 minutes (e.g., 6am, 5:30pm)" , "12hoursno0space" => "12 Hours omitting 0 minutes with space (e.g., 6 am, 5:30 pm)" );
				...
				<tr>
					...
					<td><select style='width: 250px' name='timedivision'>
							<?php $timedivisions = array(
								...
								"0.5"  => "Half-Hourly (30 min intervals)",
								"1"  => "Hourly (60 min intervals)",
								"2"  => "Bi-Hourly (120 min intervals)",
								"3"  => "Tri-Hourly (180 min intervals)"
							);
			...
			<?php elseif ( $adminpage == "categories" ): ?>
				<a href="?page=weekly-schedule&amp;settings=general&amp;schedule=<?php echo intval( $schedule ); ?>">General Settings</a> |
				<a href="?page=weekly-schedule&amp;settings=categories&amp;schedule=<?php echo intval( $schedule ); ?>"><strong>Manage Schedule Categories</strong></a> |
				<a href="?page=weekly-schedule&amp;settings=items&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Schedule Items</a> |
				<a href="?page=weekly-schedule&amp;settings=days&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Days Labels</a>
				<br /><br />
				<div style='float:left;margin-right: 15px'>
					<form name="wscatform" action="" method="post" id="ws-config">
						...
						?>
						<?php if ( $mode == "edit" ): ?>
							<strong>Editing Category #<?php echo intval( $selectedcat->id ); ?></strong><br />
						<?php endif; ?>
									...
									<?php if ( $cat->nbitems == 0 ): ?>
										<td style='background:#FFF'>
											<a href='?page=weekly-schedule&amp;deletecat=<?php echo intval( $cat->id ); ?>&schedule=<?php echo intval( $schedule ); ?>'
												<?php echo "onclick=\"if ( confirm('" . esc_js( sprintf( __( "You are about to delete this category '%s'\n  'Cancel' to stop, 'OK' to delete." ), esc_html( $cat->name ) ) ) . "') ) { return true;}return false;\"" ?>><img src='<?php echo plugins_url( '/icons/delete.png', __FILE__ ); ?>' /></a>
										</td>
									<?php else: ?>
			...
			elseif ( $adminpage == "items" ): ?>
				<a href="?page=weekly-schedule&amp;settings=general&amp;schedule=<?php echo intval( $schedule ); ?>">General Settings</a> |
				<a href="?page=weekly-schedule&amp;settings=categories&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Schedule Categories</a> |
				<a href="?page=weekly-schedule&amp;settings=items&amp;schedule=<?php echo intval( $schedule ); ?>"><strong>Manage Schedule Items</strong></a> |
				<a href="?page=weekly-schedule&amp;settings=days&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Days Labels</a>
				<br /><br />
				<div style='float:left;margin-right: 15px;width: 500px;'>
					<form name="wsitemsform" action="" method="post" id="ws-config">
						...
						<?php if ( $mode == "edit"  && isset( $selecteditem ) ): ?>
							<strong>Editing Item #<?php echo intval( $selecteditem->id ); ?></strong>
						<?php endif; ?>
				...
				<div>
					...
					if ( $items ): ?>
						<form name="wsitemdeletionform" action="?page=weekly-schedule&settings=items&schedule=<?php echo intval( $schedule ); ?>" method="post" id="ws-config">
							...
							<input class="button" type="submit" name="deleteallitems" value="Delete all items in Schedule <?php echo intval( $schedule ); ?>" onclick="return confirm('Are you sure you want to delete all items in Schedule <?php echo intval( $schedule ); ?>?')" />
						</form>
						<br />
								...
								<tr>
									...
									<td style='background:#FFF'>
										<a href='?page=weekly-schedule&amp;deleteitem=<?php echo intval( $item->id ); ?>&amp;schedule=<?php echo intval( $schedule ); ?>'
											<?php echo "onclick=\"if ( confirm('" . esc_js( sprintf( __( "You are about to delete the item '%s'\n  'Cancel' to stop, 'OK' to delete." ), esc_html( $item->name ) ) ) . "') ) { return true;}return false;\""; ?>><img src='<?php echo plugins_url( '/icons/delete.png', __FILE__ ); ?>' /></a>
									</td>
								</tr>
							<?php endforeach; ?>
			...
			elseif ( $adminpage == "days" ): ?>
				<div>
					...
					<a href="?page=weekly-schedule&amp;settings=categories&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Schedule Categories</a> |
					<a href="?page=weekly-schedule&amp;settings=items&amp;schedule=<?php echo intval( $schedule ); ?>">Manage Schedule Items</a> |
					<a href="?page=weekly-schedule&amp;settings=days&amp;schedule=<?php echo intval( $schedule ); ?>"><strong>Manage Days Labels</strong></a><br /><br />
...
class WSTodayScheduleWidget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title_url' ) ); ?>">URL for widget title:</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'empty_msg' ) ); ?>">Empty Item List Message:</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'max_items' ) ); ?>">Max Number of Items:</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'max_items' ) ); ?>">Only show later items</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'schedule_id' ) ); ?>">Schedule ID</label>
